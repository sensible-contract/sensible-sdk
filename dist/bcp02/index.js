...
exports.SensibleFT = void 0;
const scryptlib_1 = require("scryptlib");
const SatotxSigner_1 = require("../common/SatotxSigner");
const Utils = require("../common/utils");
const sensible_api_1 = require("../sensible-api");
const FungibleToken_1 = require("./FungibleToken");
const SizeHelper = require("./SizeHelper");
...
function getGenesis(txid, index) {
    const txidBuf = Buffer.from(txid, "hex").reverse();
    const indexBuf = Buffer.alloc(4, 0);
    indexBuf.writeUInt32LE(index);
    return scryptlib_1.toHex(Buffer.concat([txidBuf, indexBuf]));
}
...
class SensibleFT {
     ...
     */
    constructor({ signers = defaultSignerConfigs, feeb = 0.5, network = sensible_api_1.API_NET.MAIN, 
    ...
    mock = false, purse, }) {
        ...
        this.ft = new FungibleToken_1.FungibleToken(BigInt("0x" + signers[0].satotxPubKey), BigInt("0x" + signers[1].satotxPubKey), BigInt("0x" + signers[2].satotxPubKey));
    }
    /**
     * genesis
     * @param {Object} param0
    ...
    async genesis({ tokenName, tokenSymbol, decimalNum, genesisWif, utxos, changeAddress, opreturnData, }) {
        ...
        let genesis, codehash;
        {
            ...
            let tokenContract = this.ft.createTokenContract(genesisTxId, genesisOutputIndex, genesisContract.lockingScript, {
    ...
    async issue({ genesis, codehash, genesisWif, receiverAddress, tokenAmount, allowIncreaseIssues = true, utxos, changeAddress, opreturnData, }) {
        ...
        let issueUtxos = await this.sensibleApi.getFungbleTokenUnspents(codehash, genesis, this.zeroAddress);
        ...
        let genesisContract = this.ft.createGenesisContract(issuerPrivateKey.toPublicKey());
    ...
    async fetchFtUtxos(codehash, genesis, address) {
        let ftUtxos = await this.sensibleApi.getFungbleTokenUnspents(codehash, genesis, address);
        let cachedHexs = {};
        //获取当前花费的tx raw
        ...
        for (let i = 0; i < ftUtxos.length; i++) {
            let ftUtxo = ftUtxos[i];
            const tx = new scryptlib_1.bsv.Transaction(ftUtxo.txHex);
        ...
        ftUtxos.forEach((v) => {
            ...
            const tx = new scryptlib_1.bsv.Transaction(v.preTxHex);
            let dataPartObj = TokenProto.parseDataPart(tx.outputs[v.preOutputIndex].script.toBuffer());
            ...
            if (dataPartObj.tokenAddress == "0000000000000000000000000000000000000000") {
                ...
                v.preTokenAddress = scryptlib_1.bsv.Address.fromPublicKeyHash(Buffer.from(dataPartObj.tokenAddress, "hex"), this.network).toString();
            }
        });
        ftUtxos.forEach((v) => {
     ...
     * @returns
     */
    async transfer({ codehash, genesis, senderWif, receivers, utxos, changeAddress, isMerge, opreturnData, }) {
        checkParamGenesis(genesis);
        checkParamCodehash(codehash);
        ...
        const senderPublicKey = scryptlib_1.bsv.PublicKey.fromPrivateKey(senderPrivateKey);
        const senderAddress = senderPrivateKey.toAddress(this.network);
        //获取token
        let ftUtxos = await this.fetchFtUtxos(codehash, genesis, senderAddress.toString());
        let mergeUtxos = [];
        let mergeTokenAmountSum = BigInt(0);
        if (isMerge) {
            mergeUtxos = ftUtxos.slice(0, 20);
            mergeTokenAmountSum = mergeUtxos.reduce((pre, cur) => pre + BigInt(cur.tokenAmount), BigInt(0));
            receivers = [
                {
        ...
        let tokenOutputArray = receivers.map((v) => ({
            address: scryptlib_1.bsv.Address.fromString(v.address, this.network),
            tokenAmount: BigInt(v.amount),
        }));
        //计算输出的总金额
        let outputTokenAmountSum = tokenOutputArray.reduce((pre, cur) => pre + cur.tokenAmount, BigInt(0));
        //token的选择策略
        ...
        if (inputLength <= 3) {
            if (outputLength <= 3) {
                routeCheckType = FungibleToken_1.RouteCheckType.from3To3;
                sizeOfRouteCheck = SIZE_OF_ROUTE_CHECK_TYPE_3To3;
            }
            else if (outputLength <= 100) {
                routeCheckType = FungibleToken_1.RouteCheckType.from3To100;
            ...
            if (outputLength <= 6) {
                routeCheckType = FungibleToken_1.RouteCheckType.from6To6;
            ...
            if (outputLength <= 10) {
                routeCheckType = FungibleToken_1.RouteCheckType.from10To10;
            ...
            if (outputLength <= 3) {
                routeCheckType = FungibleToken_1.RouteCheckType.from20To3;
        ...
        let estimateSatoshis = sizeOfRouteCheck * this.feeb +
        ...
        const ftUtxoTx = new scryptlib_1.bsv.Transaction(defaultFtUtxo.txHex);
        ...
        let routeCheckContract = this.ft.createRouteCheckContract(routeCheckType, ftUtxos, tokenOutputArray, TokenProto.newTokenID(dataPartObj.tokenID.txid, dataPartObj.tokenID.index), TokenProto.getContractCodeHash(tokenLockingScript.toBuffer()));
        ...
        utxos = [
            {
                ...
                satoshis: routeCheckTx.outputs[routeCheckTx.outputs.length - 1].satoshis,
        ...
        const tokenInputArray = ftUtxos.map((v) => {
            const preTx = new scryptlib_1.bsv.Transaction(v.preTxHex);
            const preLockingScript = preTx.outputs[v.preOutputIndex].script;
            const tx = new scryptlib_1.bsv.Transaction(v.txHex);
            ...
            return {
                ...
                preLockingScript,
                preTokenAddress: new scryptlib_1.bsv.Address(v.preTokenAddress, this.network),
                preTokenAmount: v.preTokenAmount,
            };
        });
        const satoshiInputArray = utxos.map((v) => ({
            lockingScript: scryptlib_1.bsv.Script.buildPublicKeyHashOut(v.address).toHex(),
            satoshis: v.satoshis,
            txId: v.txId,
            outputIndex: v.outputIndex,
        }));
        ...
        for (let i = 0; i < sigReqArray.length; i++) {
            ...
            for (let j = 0; j < sigReqArray[i].length; j++) {
                ...
                tokenRabinPaddingArray.push(new scryptlib_1.Bytes(sigInfo.padding));
            }
    ...
    async getBalance({ codehash, genesis, address }) {
        return await this.sensibleApi.getFungbleTokenBalance(codehash, genesis, address);
    }
    /**
    ...
    async getIssueEstimateFee({ opreturnData, allowIncreaseIssues = true }) {
        ...
        let size = 4 +
            ...
            p2pkhOutputNum * (8 + 1 + 25) +
            4;
        let dust = Utils.getDustThreshold(allowIncreaseIssues ? sizeOfTokenGenesis : 0 + sizeOfToken);
        let fee = Math.ceil(size * this.feeb) + dust;
        return fee;
    }
    /**
     ...
     * @returns
     */
    async getTransferEstimateFee({ codehash, genesis, senderWif, receivers, opreturnData, }) {
        checkParamGenesis(genesis);
        checkParamCodehash(codehash);
        checkParamWif(senderWif, "senderWif");
        checkParamReceivers(receivers);
        ...
        //计算输出的总金额
        let outputTokenAmountSum = tokenOutputArray.reduce((pre, cur) => pre + BigInt(cur.tokenAmount), BigInt(0));
        //token的选择策略
        let inputTokenAmountSum = BigInt(0);
        let _ftUtxos = [];
        for (let i = 0; i < ftUtxos.length; i++) {
